...
"MF DR MC"
mf_eib_mc = MF_EIB_MCDROPOUT(num_user, num_item, pdrop = 0.7)

ips_idxs = np.arange(len(y_test))
np.random.shuffle(ips_idxs)
y_ips = y_test[ips_idxs[:int(0.05 * len(ips_idxs))]]

mf_eib_mc.fit(x_train, y_train,  y_ips=y_ips,
    lr=0.05,
    batch_size=2048,
    lamb=1e-4,
    tol=1e-5,
    verbose=False)
test_pred = mf_eib_mc.predict(x_test)
mse_mfdrmc = mse_func(y_test, test_pred)
auc_mfdrmc = roc_auc_score(y_test, test_pred)
ndcg_res = ndcg_func(mf_eib_mc, x_test, y_test)
recall_res = recall_func(mf_eib_mc, x_test, y_test)

print("***"*5 + "[MF-EIB-MC]" + "***"*5)
print("[MF-EIB-MC] test mse:", mse_mfdrmc)
print("[MF-EIB-MC] test auc:", auc_mfdrmc)
